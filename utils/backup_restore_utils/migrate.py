...
def backup(server, server_dir, local_dir, do_db_backup=True, do_files_backup=True):
    ...
    #If server is actually a website
    if not servers.get(server, create_if_needed=False):
        server_app = domains.get_server_app(server, False)

        if server_app[0]:
            server = server_app[0]
        if server_app[1]:
            server_dir = server_app[1]
    ...
    if not local_dir:
        backups_dir = smash_vars.storage_dir / "backups"
    ...
    if do_files_backup:
        ...
        flags = "zcvf" if smash_vars.verbose else "zcf"
...
[yes/No]: """.format(db_name, db_host, db_user, db_password))
        ...
        #flags = "-xvf" if smash_vars.verbose else "-xf"
    ...
    if backupFile:
        ...
        flags = "-zxvf" if smash_vars.verbose else "-zxf"
...
def migrate(from_site, to_site, migrate_db=True, migrate_files=True, use_new_db=False):
    ...
    f_flags = "-zcfv" if smash_vars.verbose else "-zcf"

    t_user = lib.webfaction.get_user(t_server)
    t_app_dir = "/home/{}/webapps/{}".format(t_user, t_app)
    t_flags = "-zxvf" if smash_vars.verbose else "-zxf"
    ...
    if migrate_files:
        ...
        try:
            ...
            #this might mean there isn't a .user.ini file so skipping
            #other it would be better to check for the file first so we don't pass over all subprocess errors
            pass
