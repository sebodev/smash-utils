...
def init(name, level):
    ...
    wpw_name = name
    wpw_level = level
    client_dir = smash_vars.google_drive_maintenance_dir / "clients" / wpw_name
...
def create_google_drive_folder():
    ...
    try:
        client_template_dir = smash_vars.google_drive_maintenance_dir / "clients" / "_client folder template"
...
# plugins_dir = smash_vars.google_drive_maintenance_dir / "Maintenance Setup" / "Plugins"
# subprocess.call(r"pscp -scp -i %UserProfile%\.ssh\sitesmash.ppk -r sebodev@webfaction:{} {}".format(plugins_dir, smash_vars.), shell=True)
# print( "copied (if everything went well) {} to {}".format(smash_vars.current_project, smash_vars.project_dir) )
# print("configuring")
