...
def create(website, server=None, app_type=CURRENT_WORDPRESS_VERSION):
    ...
    ip = socket.gethostbyname(smash_vars.servers[server]["host"])
    webfaction_user = smash_vars.servers[server]["ssh-username"]
    webfaction_passwd = smash_vars.servers[server]["ssh-password"]
