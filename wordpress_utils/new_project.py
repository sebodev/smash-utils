...
def assert_webfaction_dir_exists(the_dir):
    ...
    res = vars.servers.system(vars.wf_id, "test -d " + the_dir + " && echo Exists!!")
    if res.strip():
    ...
    # try:
    #     filestat = sftp.stat(vars.servers_theme_dir)
    # except IOError:
    #     pass
    # else:
    #     raise Exception('Whoa, we don\'t want to be overwriting code. The following theme already exists on the webfaction server: ' + the_dir)
assert_webfaction_dir_exists(vars.servers_theme_dir)
...
command = r'pscp -scp -i %UserProfile%\.ssh\sitesmash.ppk -r {} sebodev@webfaction:{}'.format(vars.project_dir, vars.servers_theme_dir)
...
#session.put_all(project_dir, vars.servers_theme_dir)
