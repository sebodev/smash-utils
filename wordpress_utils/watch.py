...
def main(project, theme=None):
    ...
    while not project:
        ...
        print(", ".join(os.listdir(str(vars.projects_root_dir))))
        print()
        project = input('Enter a project to watch: ')

    while not theme:
        print("available themes are: ")
        print(", ".join(os.listdir(str(vars.projects_root_dir / project))))
        print()
        theme = input('Enter a theme or plugin to watch, or leave blank to watch {}: '.format(vars.projects_root_dir / project))
