...
def main(old_site, new_site, csv):
    ...
    csv = Path(os.path.abspath(csv))
    tmp_csv = vars.tmp_dir/"redirects"/(old_site.replace("\\", "").replace(".", "_")+".csv")
...
def filter_redirects(in_file, out_file, old_domain, new_domain, delimiter="\t"):
    ...
    for i, line in enumerate(data[1:]):
        ...
        if skip_status_not_200 and "200" not in line[status_column] and "OK" not in line[status_column]:
            skip = True
        if url.startswith("#") or url.startswith("mailto:") or url.startswith("tel:") or url.startswith("sms:"):
            skip = True
